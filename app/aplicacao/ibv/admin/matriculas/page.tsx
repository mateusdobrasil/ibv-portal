...
export default async function MatriculasPage({ searchParams }: PageProps) {
  ...
  const temAcesso = await usuarioTemAcessoPagina(supabase, perfil?.tipo_usuario, 'ibv', 'matriculas')
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900 p-6">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          ...
          <Link href="/aplicacao/ibv/admin" className="text-sm bg-white border border-gray-200 px-4 py-2 rounded-lg font-medium shadow-sm hover:bg-gray-50 transition whitespace-nowrap">
            Voltar ao Hub
          </Link>
        </div>
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-6">
          <form method="GET" className="flex flex-col sm:flex-row gap-3 w-full lg:max-w-lg bg-white p-2 rounded-xl shadow-sm border border-gray-200">
            ...
            <div className="flex gap-2">
              ...
              {busca && (
                <Link href="/aplicacao/ibv/admin/matriculas" className="flex items-center justify-center bg-gray-100 text-gray-500 px-4 py-2.5 rounded-lg text-sm font-bold hover:bg-gray-200 hover:text-gray-700 transition">
                  Limpar
                </Link>
              )}
            </div>
          ...
          <div className="flex flex-col sm:flex-row w-full lg:w-auto gap-2 justify-end">
            ...
            <MatriculaPorTurma turmas={todasAsTurmas || []} modulo="ibv" />
          </div>
        </div>
                  ...
                  matriculasFiltradas.map((mat: any) => (
                    <tr key={mat.id} className="hover:bg-gray-50 transition">
                      ...
                      <td className="px-6 py-4 text-right space-x-3">
                        ...
                        <BotaoStatusMatricula matriculaId={mat.id} statusAtual={mat.status || 'Ativo'} modulo="ibv" />
                      </td>
                    </tr>
                  ))
                ) : (
