...
export default async function DiarioPage() {
  ...
  const temAcesso = await usuarioTemAcessoPagina(supabase, perfil?.tipo_usuario, 'ibv', 'diario')

  if (!temAcesso) {
    redirect('/aplicacao/ibv') // Se for aluno, redireciona para a área dele
  }
  ...
  const { data: turmas } = await supabase.from('ibv_turmas').select('id, nome, curso').eq('is_ebd', false).order('nome')
  const { data: materias } = await supabase.from('ibv_materias').select('id, nome, curso_id').eq('status', 'Ativa').order('nome')
  const { data: cursos } = await supabase.from('ibv_cursos').select('id, nome')
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900 p-6">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          ...
          <Link href="/aplicacao/ibv/admin" className="text-sm bg-gray-200 text-gray-700 px-4 py-2 rounded-lg font-medium hover:bg-gray-300 transition">
            Voltar ao Hub
          </Link>
        </div>

        <div className="mb-8 flex justify-end">
          <CriadorDiario alunos={alunos || []} turmas={turmas || []} materias={materias || []} cursos={cursos || []} modulo="ibv" />
        </div>
                    ...
                    return (
                      <tr key={reg.id} className="hover:bg-gray-50 transition">
                        ...
                        <td className="px-6 py-4 text-gray-600">
                          <span className="font-bold">{(reg.ibv_turmas as any)?.nome}</span><br/>
                          <span className="text-xs text-indigo-600">{(reg.ibv_materias as any)?.nome}</span>
                        </td>
                        ...
                        <td className="px-6 py-4 text-right">
                          <div className="flex justify-end items-center gap-1">
                            <CriadorDiario registro={reg} alunos={alunos || []} turmas={turmas || []} materias={materias || []} cursos={cursos || []} modulo="ibv" />
                            <BotaoExcluir id={reg.id} modulo="ibv" tipo="diario" />
                          </div>
                        </td>
                      </tr>
                    )
