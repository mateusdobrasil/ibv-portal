...
export default async function PermissoesPage({ searchParams }: PageProps) {
  ...
  const temAcesso = await usuarioTemAcessoPagina(supabase, perfil?.tipo_usuario, 'ibv', 'permissoes')
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 mb-6">
          <form method="GET" className="flex flex-col sm:flex-row gap-3">
            ...
            <div className="flex gap-2 w-full sm:w-auto">
              ...
              {busca && (
                <Link href="/aplicacao/ibv/admin/permissoes" className="flex items-center justify-center bg-gray-100 text-gray-500 px-4 py-3 rounded-lg text-sm font-bold hover:bg-gray-200 hover:text-gray-700 transition">
                  Limpar
                </Link>
              )}
            </div>
