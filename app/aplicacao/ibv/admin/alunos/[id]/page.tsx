...
export default async function PerfilAlunoAdminPage({ params }: { params: { id: string } }) {
  ...
  const temAcesso = await usuarioTemAcessoPagina(supabase, perfilLogado?.tipo_usuario, 'ibv', 'alunos')

  if (!temAcesso) {
    redirect('/aplicacao/ibv/aluno') // Impede que o aluno veja o perfil dos outros
  }
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900 p-6">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex justify-between items-start mb-8">
          <Link href="/aplicacao/ibv/admin/alunos" className="text-sm text-gray-500 hover:text-indigo-600 flex items-center gap-2 transition">
            ← Voltar para a lista
          </Link>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {matriculas.map((m: any, idx: number) => (
                      <div key={idx} className="p-4 rounded-lg border border-gray-100 bg-slate-50">
                        <p className="text-xs text-indigo-600 font-bold uppercase">{m.ibv_turmas?.curso}</p>
                        <h4 className="font-bold text-gray-800">{m.ibv_turmas?.nome}</h4>
                    ...
                    boletim.map((b: any, idx: number) => (
                      <tr key={idx}>
                        <td className="px-6 py-4">
                          <span className="block font-bold text-gray-800">{b.ibv_materias?.nome}</span>
                          <span className="text-xs text-gray-400">{b.ibv_turmas?.nome}</span>
                        </td>
