...
export default async function CadastroCentralPage({ searchParams }: PageProps) {
  ...
  const temAcesso = await usuarioTemAcessoPagina(supabase, perfil?.tipo_usuario, 'ibv', 'alunos')

  if (!temAcesso) {
    redirect('/aplicacao/ibv') // Se for Aluno ou visitante, expulsa da página
  }
                  ...
                  alunos.map((aluno) => (
                    <tr key={aluno.id} className="hover:bg-gray-50 transition">
                      ...
                      <td className="px-6 py-4 text-right">
                        <Link 
                          href={`/aplicacao/ibv/admin/alunos/${aluno.id}`} 
