...
export default async function AvisosPage() {
  ...
  const { data: turmas } = await supabase.from('ibuc_turmas').select('id, nome').order('nome')

  // 5. Busca os avisos. "avisos" é uma tabela compartilhada entre módulos e não
  // tem uma FK própria para ibuc_turmas, então o nome da turma é resolvido aqui
  // em memória a partir da lista de turmas do IBUC já carregada acima.
