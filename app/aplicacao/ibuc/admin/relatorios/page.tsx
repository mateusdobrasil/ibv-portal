...
export default async function RelatoriosPage() {
  ...
  const temAcesso = await usuarioTemAcessoPagina(supabase, perfil?.tipo_usuario, 'ibuc', 'relatorios')

  if (!temAcesso) {
    redirect('/aplicacao/ibuc/admin') 
  }
  ...
  const { count: totalTurmas } = await supabase.from('ibuc_turmas').select('*', { count: 'exact', head: true })
  const { count: totalMatriculas } = await supabase.from('ibuc_matriculas').select('*', { count: 'exact', head: true })

  const { data: financeiro } = await supabase.from('ibuc_financeiro').select('valor, status')
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div className="flex gap-3">
            <Link href="/aplicacao/ibuc/admin/relatoriosEBD" className="text-sm bg-indigo-50 border border-indigo-100 text-indigo-700 px-5 py-2.5 rounded-xl font-bold hover:bg-indigo-100 transition shadow-sm">
              Ver Relatório da EBD ➡️
            </Link>
            <Link href="/aplicacao/ibuc/admin" className="text-sm bg-white border border-gray-200 text-gray-700 px-5 py-2.5 rounded-xl font-bold hover:bg-gray-50 transition shadow-sm">
              Voltar ao Hub
            </Link>
          </div>
        </div>
