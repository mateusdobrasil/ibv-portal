...
export default async function FinanceiroPage() {
  ...
  const temAcesso = await usuarioTemAcessoPagina(supabase, perfil?.tipo_usuario, 'ibuc', 'financeiro')

  if (!temAcesso) {
    redirect('/aplicacao/ibuc/admin') // Redireciona o Professor de volta para o Hub
  }
                      ...
                      <tr key={cob.id} className="hover:bg-gray-50 transition">
                        ...
                        <td className="px-6 py-4 text-right">
                          <div className="flex justify-end items-center gap-1">
                            {cob.status === 'Pendente' && (
                              <BotaoBaixarCobranca cobrancaId={cob.id} modulo="ibuc" />
                            )}
                            <CriadorFinanceiro cobranca={cob} alunos={alunos || []} modulo="ibuc" />
                            <BotaoExcluir id={cob.id} modulo="ibuc" tipo="fatura" />
                          </div>
                        </td>
                      </tr>
                    )
