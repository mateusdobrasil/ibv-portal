...
export default async function CadastroCentralPage({ searchParams }: PageProps) {
  ...
  const temAcesso = await usuarioTemAcessoPagina(supabase, perfil?.tipo_usuario, 'ibuc', 'alunos')

  if (!temAcesso) {
    redirect('/aplicacao/ibuc') // Se for Aluno ou visitante, expulsa da página
  }
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-6">
          <form method="GET" className="flex flex-col sm:flex-row gap-3 w-full lg:max-w-lg bg-white p-2 rounded-xl shadow-sm border border-gray-200">
            ...
            <div className="flex gap-2">
              ...
              {busca && (
                <Link href="/aplicacao/ibuc/admin/cadastro" className="flex items-center justify-center bg-gray-100 text-gray-500 px-4 py-2.5 rounded-lg text-sm font-bold hover:bg-gray-200 hover:text-gray-700 transition">
                  Limpar
                </Link>
              )}
            </div>
                  ...
                  alunos.map((aluno) => (
                    <tr key={aluno.id} className="hover:bg-gray-50 transition">
                      ...
                      <td className="px-6 py-4 text-right">
                        <Link 
                          href={`/aplicacao/ibuc/admin/alunos/${aluno.id}`} 
